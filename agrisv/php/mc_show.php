...
function get_sqlString($vNum , $getDat){
	$ret="";
	$sql = "select id , snum, created,DATE_FORMAT(created,'%H%i') as ct_str ,DATE_FORMAT(now(),'%m%d')  from t_sv_sensor";
...
include_once("../libs/AppCom.php");
	...
	if(isset($_GET["id"])){
	  if(isset($_GET["mmdd"])){
	  	$datMc = com_getMcDat($db, $_GET["id"] ,$_GET["sv_id"]);
	  	if($datMc !=NULL){
			$i_ct =0;
			$dat = array();
